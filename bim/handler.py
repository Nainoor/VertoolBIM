...
def get_application(ifc):
    ...
    for element in ifc.by_type("IfcApplication"):
        if element.ApplicationIdentifier == "VertoolBIM" and element.Version == version:
            return element
    return ifcopenshell.api.run(
        "owner.add_application",
        ifc,
        version=version,
        application_full_name="VertoolBIM Add-on",
        application_identifier="VertoolBIM",
    )
    ...
    return ".".join(
        [
            ...
            for x in [
                ...
                if addon.bl_info["name"] == "VertoolBIM"
            ][0]
        ]
    )
